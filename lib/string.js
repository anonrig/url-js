...
function utf8_percent_encode(input, predicate) {
  ...
  for (const code_point of bytes) {
    let character = String.fromCodePoint(code_point);
...
function utf8_percent_encode_string(input, predicate, space_as_plus = false) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    if (space_as_plus && code === 32) {
      output += "+";
    } else {
      output += utf8_percent_encode(code, predicate);
    }
  }
  return output;
}
