 ...
 * @description A string starts with a Windows drive letter if all of the following are true:
...
function starts_with_windows_drive_letter(input, pointer) {
  ...
  return length >= 2 &&
  (
    is_windows_drive_letter(input[pointer + 1]) && is_windows_drive_letter(
      input[pointer + 2],
    )
  ) &&
