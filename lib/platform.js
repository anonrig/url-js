...
const file_code_points = new Set(['/', '\\', '?', '#'])
...
function starts_with_windows_drive_letter(input, pointer) {
  ...
  return length >= 2 &&
    is_windows_drive_letter_code(input[pointer].codePointAt(0), input[pointer + 1]) &&
    (length === 2 || file_code_points.has(input[pointer + 2]))
}
...
function is_normalized_windows_drive_letter(input) {
  return input.length === 2 && is_ascii_alpha(input.codePointAt(0)) && input[1] === ':'
}
