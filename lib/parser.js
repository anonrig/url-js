...
const { utf8_percent_encode_string, is_ascii_hex, is_ascii_digit } = require(
  "./string",
);
...
function parse_ipv4(input) {
  ...
  if (parts[parts.length - 1] >= 256 ** (5 - parts.length)) {
    return FAILURE;
  }
...
function parse_host(buffer, is_not_url_special = false) {
  ...
  if (buffer.codePointAt(0) === 91) {
    // If input does not end with U+005D (]), validation error, return failure.
    if (buffer.codePointAt(buffer.length - 1) !== 93) {
      return FAILURE;
    }
  ...
  const domain = Buffer.from(buffer, "utf-8").toString("utf-8");
  const ascii_domain = domainToASCII(domain);
