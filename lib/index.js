...
const { is_user_info_percent_encode_set, is_fragment_percent_encoded } = require("./encoding")
...
URLStateMachine.prototype.fragmentState = function fragmentState(code, character) {
  if (code && !Number.isNaN(code)) {
    ...
    if (character !== '%') {
      this.hasValidationError = true
    } else if (character === '%' && is_ascii_hex(this.input.codePointAt(this.pointer + 1)) && is_ascii_hex(this.input.codePointAt(this.pointer + 2))) {
    ...
    this.url.fragment += utf8_percent_encode(character, is_fragment_percent_encoded)
  }
}
