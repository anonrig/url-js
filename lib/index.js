...
function URLStateMachine(input, base = null, encodingOverride, stateOverride) {
  ...
  while (
    ...
    this.processState(this.input.codePointAt(this.pointer))
  ) {
    this.pointer++;
  }
}
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.processState = function processState(code) {
  switch (this.state) {
 ...
 * @returns {void}
 */
URLStateMachine.prototype.shortenUrl = function shortenUrl() {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.authorityState = function authorityState(code) {
  // If c is U+0040 (@), then:
  if (code === 64) {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.hostState = function hostState(code) {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.schemeStartState = function schemeStartState(code) {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.schemeState = function schemeState(code) {
  ...
  // Otherwise, if c is U+003A (:), then:
    ...
    if (this.stateOverride) {
      const buffer_special =
        typeof special_schemes[this.buffer] !== "undefined";
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.noSchemeState = function noSchemeState(code) {
  // If base is null, or base has an opaque path and c is not U+0023 (#), validation error, return failure.
  if (
    this.base === null ||
    (typeof this.base.path === "string" && code !== 35)
  ) {
    this.failure = true;
    return false;
  }
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.fragmentState = function fragmentState(code) {
  ...
  if (code) {
    ...
    this.url.fragment = utf8_percent_encode(
      this.input.slice(this.pointer),
      is_fragment_percent_encoded,
    );
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.relativeState = function relativeState(code) {
...
URLStateMachine.prototype.specialAuthoritySlashesState =
  function specialAuthoritySlashesState(code) {
    if (code === 47 && this.input[this.pointer + 1] === "/") {
      ...
      // Otherwise, validation error, decrease pointer by 1.
      this.pointer--;
    }
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.pathOrAuthorityState = function pathOrAuthorityState(
  code,
) {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.relativeSlashState = function relativeSlashState(
  code,
) {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.fileState = function fileState(code) {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.fileHostState = function fileHostState(code) {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.fileSlashState = function fileSlashState(code) {
  // If c is U+002F (/) or U+005C (\), then:
  if (code === 47 || code === 92) {
  ...
  // Otherwise:
    ...
    if (this.base !== null && this.base.scheme === "file") {
      ...
      if (
        !starts_with_windows_drive_letter(this.input, this.pointer) &&
        is_normalized_windows_drive_letter(this.base.path[0])
      ) {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.queryState = function queryState(code) {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.pathStartState = function pathStartState(code) {
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.pathState = function pathState(code) {
  // If one of the following is true:
  ...
  // Otherwise, run these steps:
    ...
    this.buffer += utf8_percent_encode(
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.opaquePathState = function opaquePathState(code) {
  // If c is U+003F (?), then set url’s query to the empty string and state to query state.
  if (code === 63) {
    this.url.query = "";
    this.state = State.QUERY;
  ...
  // Otherwise:
    ...
    if (!isNaN(code)) {
      this.url.path += utf8_percent_encode(
        this.input[this.pointer],
        is_c0_control_percent_encoded,
      );
    }
 ...
 * @returns {boolean}
 */
URLStateMachine.prototype.portState = function portState(code) {
...
URLStateMachine.Constants = { FAILURE, State, Regex };
