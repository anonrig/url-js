...
/**
 * @typedef {'scheme-start'|'no-scheme'|'scheme'|'fragment'|'relative'|'relative-slash'|'file'|'file-slash'|'query'|'path'|'file-host'|'path-or-authority'|'special-relative-or-authority'} State
 *
 * @class
 * @public
 * @constructor
...
URLStateMachine.prototype.schemeState = function schemeState(code, character) {
  if (is_ascii_alphanumeric(code) || ['+', '-', '.'].includes(character)) {
    ...
    if (this.stateOverride) {
      const url_scheme_special = special_schemes.includes(this.url.scheme)
      const buffer_special = special_schemes.includes(this.buffer)
