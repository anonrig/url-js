...
const { is_user_info_percent_encode_set, is_fragment_percent_encoded } = require(
  "./encoding",
);
    ...
    if (character === "/" || character === "\\") {
      ...
      if (character === "?") {
        ...
        if (!is_windows_drive_letter_code(code, character)) {
          ...
          if (this.url.scheme === 'file' && this.url.path.length === 1 && is_normalized_windows_drive_letter(this.url.path[0])) {
            // Do nothing.
          } else {
