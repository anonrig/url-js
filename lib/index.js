...
/**
 * @typedef {'authority'|'scheme-start'|'no-scheme'|'scheme'|'fragment'|'relative'|'relative-slash'|'file'|'file-slash'|'query'|'path'|'file-host'|'path-or-authority'|'special-relative-or-authority'|'special-authority-slashes'|'special-authority-ignore-slashes'|'path-start'|'opaque-path'} State
 *
 * @class
 * @public
 * @constructor
...
function URLStateMachine(input, base, stateOverride) {
  ...
  while (this.pointer < this.input.length) {
    ...
    const code = this.input.codePointAt(this.pointer);
...
URLStateMachine.prototype.setInput =
  function setInput(input) {
    ...
    const control_space_removed = input.replace(
      /[\u0000-\u001F\u007F-\u009F]/g,
      "",
    );
...
URLStateMachine.prototype.hostState =
  function hostState(code, character) {
    ...
    // - url is special and c is U+005C (\)
      ...
      (Number.isNaN(code) || character === "/" || character === "?" || character === "#") || (
        this.isSpecialUrl && character === "\\"
      )
    ) {
      this.pointer--;
    ...
    if (
      ...
      if (this.stateOverride) {
        ...
        if (this.isSpecialUrl && !buffer_special) {
          ...
          (
            (this.url.username.length > 0 && this.url.password.length > 0) || this.url.port !== null
          ) && this.buffer === "file"
        ) {
      ...
      if (this.url.scheme === "file") {
        ...
        if (
          this.input[this.pointer + 1] !== "/" || this.input[this.pointer + 2] !== "/"
        ) {
          this.hasValidationError = true;
          this.state = "file";
        }
...
URLStateMachine.prototype.fragmentState =
  function fragmentState(code, character) {
    ...
    if (!Number.isNaN(code)) {
      ...
      if (
        character === "%" &&
        !is_ascii_hex(this.input.codePointAt(this.pointer + 1)) &&
        !is_ascii_hex(this.input.codePointAt(this.pointer + 2))
      ) {
        // If c is U+0025 (%) and remaining does not start with two ASCII hex digits, validation error.
        this.hasValidationError = true;
      }
...
URLStateMachine.prototype.queryState =
  function queryState(code, character) {
    ...
    if ((!this.stateOverride && character === "#") || Number.isNaN(code)) {
    ...
    // Otherwise, if c is not the EOF code point:
      ...
      // If c is not a URL code point and not U+0025 (%), validation error.
      if (character !== "#" && !is_ascii_alphanumeric(code)) {
        this.hasValidationError = true;
      }
...
URLStateMachine.prototype.pathState =
  function pathState(code, character) {
    ...
    if (
      ...
      if (
        this.buffer === ".." ||
        this.buffer.toLowerCase() === "%2e." ||
        this.buffer.toLowerCase() === ".%2e" ||
        this.buffer.toLowerCase() === "%2e%2e"
      ) {
        // Shorten url’s path.
        this.shortenUrl();
      ...
      // Otherwise, if buffer is not a single-dot path segment, then:
      else if (this.buffer !== "." && this.buffer.toLowerCase() !== "%2e") {
        ...
        if (
          ...
          is_normalized_windows_drive_letter(this.buffer)
        ) {
...
URLStateMachine.prototype.opaquePathState =
  function opaquePathState(code, character) {
    ...
    // Otherwise:
      ...
      if (!Number.isNaN(code) && !is_ascii_alphanumeric(code) && character !== "%") {
        this.hasValidationError = true;
      }
