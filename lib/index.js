...
URLStateMachine.prototype.authorityState = function authorityState(code, character) {
  ...
  if (character === '@') {
    ...
    this.buffer = ''
  } else if (character === '/' || character === '?' || character === '#' || isNaN(code) || (special_schemes[this.url.scheme] !== undefined && character === '\\')) {
...
URLStateMachine.prototype.hostState = function hostState(code, character) {
  ...
  if (this.stateOverride && this.url.scheme === 'file') {
    ...
    const host = parse_host(this.buffer, special_schemes[this.url.scheme] === undefined)
    ...
    this.buffer = ''
    this.state = 'port'
  } else if ((character === '/' || character === '?' || character === '#') || special_schemes[this.url.scheme] !== undefined && character === '\\') {
    // Otherwise, if one of the following is true:
    // - c is the EOF code point, U+002F (/), U+003F (?), or U+0023 (#)
    // - url is special and c is U+005C (\)
    this.pointer--
    ...
    if (special_schemes[this.url.scheme] !== undefined && this.buffer === '') {
      this.hasValidationError = true
      return FAILURE
...
URLStateMachine.prototype.relativeState = function relativeState(code, character) {
  ...
  } else if ( special_schemes[this.url.scheme] !== undefined && character === '\\') {
...
URLStateMachine.prototype.relativeSlashState = function relativeSlashState(code, character) {
  ...
  if (special_schemes[this.url.scheme] !== undefined && (character === '/' || character === '\\')) {
