...
/**
 * @typedef {'scheme-start'|'no-scheme'|'scheme'|'fragment'|'relative'|'relative-slash'|'file'|'file-slash'|'query'} State
 *
 * @class
 * @public
 */
class URLStateMachine {
  ...
  noSchemeState(code, character) {
    const has_opaque_path = this.base?.path === 'string'
    // If base is null, or base has an opaque path and c is not U+0023 (#), validation error, return failure.
    if (this.base === null || (this.base.path === 'string' && character !== '#')) {
      this.hasValidationError = true
      return FAILURE
    } else if (this.base.path === 'string' && character === '#') {
